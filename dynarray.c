...
{
    struct dynarray* cs261 = malloc(sizeof(struct dynarray));
    cs261->data = malloc(2 * sizeof (void*));
    cs261->size = 0;
    cs261->capacity = 2;
...
{
    // Removes an element at a given index from the array, elements after it should be pushed forward
...
{
    // Gets the value of the element stored at a given index in the array
    return da->data[idx];
}
...
{
    // Sets/updates the value of the element stored at a given index in the array
    da->data[idx] = val;
}
